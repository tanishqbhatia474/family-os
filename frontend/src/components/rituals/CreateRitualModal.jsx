      ...
      <div className="flex justify-between items-start gap-3 sm:gap-4">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-lg sm:text-2xl font-bold text-[var(--text)] truncate">{title}</h2>
            <p className="text-xs sm:text-sm text-[var(--muted)] mt-0.5 sm:mt-1 line-clamp-2">{subtitle}</p>
          </div>
        </div>
        <button
          onClick={onClose}
...
function PersonCheckbox({ person, checked, onToggle }) {
  return (
    <label className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg sm:rounded-xl hover:border-[var(--accent)] border border-transparent cursor-pointer">
      <input 
        type="checkbox" 
        checked={checked} 
        onChange={onToggle}
      ...
      <span className="text-xs sm:text-sm font-medium text-[var(--text)] truncate">{person.name}</span>
    </label>
  );
}
